...
(define preview-render%
  (class render%
    ...
    (define/override (render-paragraph p part ri)
      ;; A "TonartRule" paragraph is an empty content-less marker that
      ;; renders as a thick black rule (it brackets an embedded
      ;; art-program).  It has no spans, so key off the style name.
      (when (equal? (style-name* (paragraph-style p)) "TonartRule")
        (emit-block! '(rule)))
      (define spans
        (if (equal? (style-name* (paragraph-style p)) "TonartRule")
            '()
            (capture-spans (lambda () (super render-paragraph p part ri)))))
      (unless (null? spans)
