...
(preferences:set-default 'programmart4-realizer-preview:volume 2
                         (lambda (x) (and (exact-integer? x) (<= 1 x 10))))

;; Output path for playback: 'audio (SawOsc -> dac) or 'midi (MidiOut
;; -> "IAC Driver Bus 1").
(preferences:set-default 'programmart4-realizer-preview:out 'audio
  ...
  (class vertical-panel%
    ...
    (define out-choice
      (new radio-box% [label #f] [parent bar]
           [choices '("Tone" "MIDI")]
           [style '(horizontal)]
           [selection (if (eq? 'midi (preferences:get 'programmart4-realizer-preview:out)) 1 0)]
           [callback (lambda (r e)
                       (preferences:set 'programmart4-realizer-preview:out (out-mode))
                       (sync-out-controls!))]))
    ...
    (define volume-slider
      (new slider% [label "Vol"] [parent bar]
           ...
           [init-value (preferences:get 'programmart4-realizer-preview:volume)]
           [min-width 70] [stretchable-width #f]
           [callback (lambda (s e)
                       (preferences:set 'programmart4-realizer-preview:volume (send s get-value)))]))
    ...
    (define/public (ensure-rendered)
      (unless (or blocks in-flight?)
        (render-message! doc-text
                         "Scribble renders the program as a document here \u2014 it realizes the module's `program` art.\n\nStrudel \u25b8 and Play arrive with the strudel and chuck realizers; for now they report that."
                         zoom)))
