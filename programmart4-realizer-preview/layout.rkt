...
;; ---------------------------------------------------------------------------
;; a thick black horizontal rule (brackets an embedded art-program)
;; ---------------------------------------------------------------------------

(define rule-snip%
  (class snip%
    (init-field [rule-width 0.0] [rule-height 3.0])
    (super-new)
    (define/override (get-extent dc x y [w #f] [h #f] [descent #f]
                                 [space #f] [lspace #f] [rspace #f])
      (define (b! bx v) (when bx (set-box! bx v)))
      (b! w (exact->inexact rule-width))
      (b! h (exact->inexact rule-height))
      (b! descent 0.0) (b! space 0.0) (b! lspace 0.0) (b! rspace 0.0))
    (define/override (draw dc x y left top right bottom dx dy draw-caret)
      (define old-brush (send dc get-brush))
      (define old-pen (send dc get-pen))
      (send dc set-pen "black" 0 'transparent)
      (send dc set-brush "black" 'solid)
      (send dc draw-rectangle x y rule-width rule-height)
      (send dc set-brush old-brush)
      (send dc set-pen old-pen))
    (define/override (copy)
      (new rule-snip% [rule-width rule-width] [rule-height rule-height]))))
...
(define (insert-block! c b #:break? [break? #t])
  ...
  (case (car b)
    ...
    [(rule)
     (put-snip! c (new rule-snip%
                       [rule-width (max 1.0 (- (ctx-width c) 4))]
                       [rule-height (* 3 zoom)]))
     (when break? (newline! c))]
